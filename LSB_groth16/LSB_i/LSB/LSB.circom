...
template LSB (numPixels) {
    ...
    signal input binaryWatermark[numPixelscheng3];
    ...
    for (var i = 0; i < hownumberPixels; i++) {
        for (var j = 0; j < 3; j++) {
...
template Main () {
    ...
    signal input originalPixelValues[hownumberPixels][3];
    signal input Watermark_PixelValues[hownumberPixels][3];
    signal input binaryWatermark[numPixelscheng3];
    signal input binaryWatermark_num;

    component lsb = LSB(hownumberPixels);
