...
subprocess.run("snarkjs groth16 setup LSB.r1cs ptau/pot16_final.ptau zkey/LSB_000.zkey", shell=True)
...
# 3. 验证
start_time = time.time()
subprocess.run("snarkjs zkey verify LSB.r1cs ptau/pot16_final.ptau zkey/LSB_001.zkey", shell=True)
...
# 5. 验证最终的zkey
start_time = time.time()
subprocess.run("snarkjs zkey verify LSB.r1cs ptau/pot16_final.ptau zkey/LSB_final.zkey", shell=True)
